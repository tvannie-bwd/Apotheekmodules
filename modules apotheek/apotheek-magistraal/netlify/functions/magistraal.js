...
export default async (req) => {
  const verwachtToken = process.env.APOTHEEK_TOKEN;
  if (verwachtToken) {
    // Enkel controleren als er effectief een token is geconfigureerd.
    // Zonder APOTHEEK_TOKEN (bv. tijdens lokaal development) staat de
    // API open, zodat je makkelijker kan testen zonder eerst in te loggen.
    const meegestuurdToken = (req.headers.get("authorization") || "").replace(/^Bearer\s+/i, "");
    if (meegestuurdToken !== verwachtToken) {
